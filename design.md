...
| 5 | [时间索引](docs/design/time-index.md) | TimeIndex 生命周期、IndexSegment 二分查找、32 字节固定 IndexEntry 序列化 | 索引查询优化 |
...
| 21 | [Dataset Identifier](docs/design/dataset-identifier.md) | Store 级 `max_identifier`、dataset 级 `identifier` 文件、按 id 打开/重命名 dataset API、crash 边界 | 外部系统需要稳定数字 dataset id |
...
| W-DOTNET | [\.NET Wrapper](wrapper/dotnet/design.md) | UniFFI C# binding、C# facade、.NET 8+、NuGet 包装、queue/journal 支持 | .NET 集成 |
...
- **数据保留回收**: `retention_window == 0` 时禁用。`timestamp_units_per_seconds == 0` 保持基于 `latest_written_timestamp` 的 legacy 阈值; 非零值使用当前 Unix seconds × units 的 wall-clock 阈值、严格过期边界和持久化单调 floor。详见 [数据集操作·§11](docs/design/dataset-operations.md#十一数据保留-retention-与回收) → [后台任务·§17.8](docs/design/background-and-cache.md#178-retention-reclaim-数据保留回收)。
...
| 模块 | 对应文档 |
|------|---------|
| `Store` + `create_dataset` + `open_dataset` + `rename_dataset` | [Store 与 C ABI Wrapper](docs/design/store-and-ffi.md) |
...
| `wrapper/dotnet` | [.NET Wrapper](wrapper/dotnet/design.md) |
