...
- **数据保留回收**: `retention_window == 0` 时禁用。`timestamp_units_per_second == 0` 保持基于 `latest_written_timestamp` 的 legacy 阈值; 非零值使用当前 Unix seconds × units 的 wall-clock 阈值、严格过期边界和持久化单调 floor。详见 [数据集操作·§11](docs/design/dataset-operations.md#十一数据保留-retention-与回收) → [后台任务·§17.8](docs/design/background-and-cache.md#178-retention-reclaim-数据保留回收)。
