...
| **Journal** | 默认启用的专用 `.journal/logs` append log 记录变更日志 | 删除无用 index 分段, 使用连续 sequence + JournalSegment/JournalQueue; 可通过专用 read/query/open_queue API 消费; 可通过 `enable_journal=false` 关闭; 不升级为事务 WAL |
