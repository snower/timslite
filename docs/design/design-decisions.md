...
|--------|------------------|-----------------|
| 存储单元 | 单条 record | Block (多条聚合, 未压缩 payload ≤256KiB) |
| 压缩粒度 | record | Block |
...
| 索引条目 | 16B (ts+offset) | 32B (`timestamp`+block+4-byte in-block offset units+reserved) |
...
| Block 上限 | 未压缩 payload 256KiB | 允许更多 record 聚合; 这是内容上限，不是 Block 固定磁盘预分配 |
...
| Record 编码 | data_len(4)+ts(8)+data+zero padding | record stored span 按 4 字节对齐，支持以 4 字节单位定位 block 内 record |
| 索引条目 | 32 字节 | 保存完整 `i64` timestamp、逻辑 block offset、4 字节单位 in-block offset 和 14 个零 reserved bytes |
...
| flush 间隔 | 可配置, 默认 15s | 平衡数据持久化与性能；10min 是早期草案值，不再作为默认 |
...
| **Journal** | 默认启用的专用 `.journal/logs` append log 记录变更日志 | 删除无用 index 分段, 使用连续 sequence + JournalSegment/JournalQueue; 可通过专用 read/query/open_queue API 消费; 可通过全局 `StoreConfig.enable_journal=false` 关闭, 或通过 dataset 级 `DataSetConfig.enable_journal=false` 跳过单个 dataset; 不升级为事务 WAL |
