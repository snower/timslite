...
pub fn poll(&self, timeout: Duration) -> Result<Option<(i64, Vec<u8>)>> {
    ...
    //    advance processed_ts through the continuous completed prefix.
...
重投前先检查 `max_retry_count`: 如果 `max_retry_count > 0 && retry_count >= max_retry_count`, 该 entry 标记为完成并按连续完成前缀推进 `processed_ts`, 不再返回给 consumer; 否则先递增 `retry_count` 并刷新 `start_time`, 再返回对应数据。
