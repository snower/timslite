...
| 任务 | 间隔 | 行为 |
|------|------|------|
| Flush | 可配置, 默认 15s | drain Store 级共享待 flush 队列同步普通 dataset dirty data/index/queue state/dataset state; 直接调用 JournalManager 同步 dirty journal segment / journal queue state |
...
创建下一 data segment 时还必须把旧 active tail 的统计累加到 dataset state 文件并入队 `SegmentFlushTarget::DatasetState`; 创建下一 index segment 时把旧 active index segment 的 timestamp 范围合并到 dataset state 文件并入队。这个直接 flush 不改变 pending block 状态, 只减少已完结分段在后台 flush 间隔内的异常窗口。
