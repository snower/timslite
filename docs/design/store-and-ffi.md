...
`timestamp_units_per_seconds: u64` 是创建时确定并持久化的 dataset 配置, 默认 `0`。`0` 保持按 `latest_written_timestamp` 计算的 legacy retention。非零值定义每个 Unix 秒对应的 dataset timestamp units, 使 `retention_window` 按 wall-clock 时间过期; `write_now` 和 `append_now` 使用相同缩放后的时间域。
