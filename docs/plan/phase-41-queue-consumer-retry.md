...
> 目标: 为普通 DatasetQueue 与 JournalQueue 增加消费组级 visibility timeout 和 retry limit, 并将 queue state file 升级为 QSTF v2 以持久化 `retry_count`。
...
Phase 41 将 pending entry 升级为 18B, 新增 `retry_count`, 并把 timeout 逻辑从后台 cleanup 移到 poll 流程中。

---

## 设计决策

- QSTF version 升级为 `2`, `pending_value_size=18`, 最大 pending entries 从 `239` 改为 `226`。
...
- [x] 测试 RED — QSTF v2 18B roundtrip、未过期不重投、过期重试递增、retry 超限丢弃、组级配置冲突、JournalQueue 同步语义
- [x] Rust 实现 — `QueueConsumerConfig` builder、state file v2、retryable pending 扫描、processed 水位推进、后台 flush 去 timeout cleanup
