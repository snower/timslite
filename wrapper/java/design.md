...
- Maven compiler `release`: `8` (`source`/`target` fallback: `8`)
- Kotlin Maven plugin `jvmTarget`: `1.8`, with `-Xjdk-release=1.8`
...
- Maven compiler `release` 使用 `8`, 并保留 `source`/`target=8` 作为 JDK 8 fallback。
- Kotlin Maven plugin `jvmTarget` 使用 `1.8`, 并通过 `-Xjdk-release=1.8` 约束 JDK API 面。
