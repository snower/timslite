...
/**
 * A consumer that reads records from a queue within a specific consumer group.
 *
...
public final class QueueConsumer implements AutoCloseable {
    ...
    static {
        try {
            POLL_METHOD = QueueConsumerBridge.class.getMethod("poll-VKZWuLQ", long.class);
        } catch (NoSuchMethodException e) {
            throw new RuntimeException("Failed to find QueueConsumerBridge.poll method", e);
        }
    }

    private final QueueConsumerBridge bridge;
    private boolean closed;

    QueueConsumer(QueueConsumerBridge bridge) {
        this.bridge = bridge;
        this.closed = false;
    }
    ...
    private void checkNotClosed() {
        if (closed) {
            throw new IllegalStateException("QueueConsumer is closed");
        }
    }
     ...
     * @return the next record, or {@code null} if the timeout expires
     * @throws TmslException if polling fails
     */
    public Record poll(long timeoutMs) {
        checkNotClosed();
        try {
            io.github.snower.timslite.uniffi.Record kotlinRecord =
                    (io.github.snower.timslite.uniffi.Record) POLL_METHOD.invoke(bridge, timeoutMs);
    ...
    /**
     * Acknowledges a record, advancing the consumer position past its timestamp.
     *
     * @param timestamp the timestamp of the record to acknowledge
