...
| 多 Python 版本 | ✅ 已解决 — ci.yml 覆盖 Python 3.9-3.13 |
...
- [x] 开发期 `wrapper/python/Cargo.toml` 使用 `timslite = { path = "../..", version = "=0.1.2" }`, 继续直接引用仓库根 crate, 并用精确版本防止 root crate / wrapper crate / Python package 版本漂移。
- [x] PyPI sdist 构建前运行 `wrapper/python/scripts/prepare_publish.py`, 将依赖改写为 `timslite = { version = "=0.1.2" }`, 让未命中预编译 wheel 的安装可从 crates.io 拉取同版本 Rust crate 源码构建。
...
### Phase PY-1: 项目骨架 + 构建系统 ✅ 完成

- [x] `wrapper/python/Cargo.toml` — 创建 maturin/PyO3 crate
  ...
  - `[dependencies] timslite = { path = "../..", version = "=0.1.2" }`
- [x] `wrapper/python/pyproject.toml` — PEP 517 构建配置
  - `build-system: requires = ["maturin>=1.0,<2.0"]`, `build-backend = "maturin"`
  - `[project]` name = "timslite", version = "0.1.2", requires-python = ">=3.9"
  - `[tool.maturin]` features = `["pyo3/extension-module"]`
- [x] `wrapper/python/src/lib.rs` — PyO3 模块入口
  - `use pyo3::prelude::*`
  - `#[pymodule] fn timslite(m: &Bound<'_, PyModule>) -> PyResult<()>` 
  - 注册所有 `#[pyclass]` 和自定义异常
  - 暂不包含实现, 仅声明模块
- [x] `wrapper/python/python/timslite/__init__.py` — 最小导出
  - `from .timslite import *` (导入编译后的 .so/.pyd 扩展)
- [x] 验证: `cd wrapper/python && maturin develop` 成功
- [x] 验证: `python -c "import timslite; print(dir(timslite))"` 显示模块
- [x] 验证: `cd wrapper/python && maturin develop --release` 编译成功
...
### Phase PY-2: 异常层次 + 错误映射 ✅ 完成

- [x] `wrapper/python/src/exceptions.rs` — 定义 9 个自定义异常
...
- [x] 实现 `fn map_error(err: timslite::TmslError) -> PyErr`
...
- [x] 在 `lib.rs` 中注册异常到模块
- [x] 验证: 每个异常可在 Python 中捕获 (`except timslite.TmslNotFoundError`)
- [x] 验证: 异常携带正确的错误消息字符串
- [x] 单元测试: `test_basic.py` — 验证所有异常可导入, 继承关系正确
...
### Phase PY-3: StoreConfig + Store (核心) ✅ 完成

#### 3.1 StoreConfig

- [x] `wrapper/python/src/config.rs` — `PyStoreConfig`
...
- [x] `wrapper/python/src/store.rs` — `PyStore`
...
- [x] 验证: `with Store.open(tmpdir) as store:` 进入/退出正常
- [x] 验证: 退出时自动调用 close (数据持久化)
- [x] 验证: 手动调用 `store.close()` 后再调用操作 → 友好错误
...
- [x] 验证: `store.create_dataset("sensor", "data")` 成功
- [x] 验证: `store.create_dataset("sensor", "data")` 第二次 → `TmslAlreadyExistsError`
- [x] 验证: `ds.write(1, b"hello")` 写入成功
- [x] 验证: `ds.write(-1, b"bad")` → `TmslInvalidDataError`
- [x] 验证: `store.open_dataset("nonexistent", "data")` → `TmslNotFoundError`
...
### Phase PY-5: 查询迭代器 (`Arc<DataSet>` 模式) ✅ 完成

- [x] `wrapper/python/src/query.rs` — `PyQueryIterator`
  - `#[pyclass]` 结构体:
    ```rust
...
- [x] 在 `PyDataset::query()` 中创建 `PyQueryIterator`:
  - 调用 `DataSet::query_index_entries(start_ts, end_ts)` 收集条目
  - 克隆 `Arc<DataSet>`
  - 返回 `PyQueryIterator { entries, dataset_arc, index: 0 }`
- [x] 验证: `for ts, data in ds.query(1, 100)` 正确迭代
- [x] 验证: `list(ds.query(1, 100))` 收集为完整列表
- [x] 验证: 空查询范围 → 立即 `StopIteration`
- [x] 验证: filler entries 被正确跳过 (连续模式)
- [x] 验证: 迭代中途关闭 store → 优雅处理 (不会 panic)
...
### Phase PY-6: Python 模块导出 + `__init__.py` ✅ 完成

- [x] 完善 `wrapper/python/src/lib.rs` — 完整模块注册
  ```rust
  #[pymodule]
  ...
  ```
- [x] 完善 `wrapper/python/python/timslite/__init__.py`
  - 从编译后的扩展模块导入所有公共符号
  ...
  ```
- [x] 验证: `import timslite` + `dir(timslite)` 包含所有符号
- [x] 验证: 所有文档字符串 (`__doc__`) 正确显示
- [x] 验证: `from timslite import Store, Dataset, StoreConfig` 正常
...
### Phase PY-7: 集成测试 (8 个测试文件) ✅ 完成

- [x] `tests/test_basic.py` — Smoke 测试
  - [x] `test_import` — `import timslite` 成功, 模块有所有导出符号
  - [x] `test_store_open_close` — `Store.open(tmpdir)`, `close()` 无错误
  - [x] `test_store_context_manager` — `with Store.open(tmpdir): pass` 正常退出
- [x] `tests/test_lifecycle.py` — 生命周期测试
  - [x] `test_create_open_write_close` — 完整 create→open→write→close 流程
  - [x] `test_create_twice_raises` — 重复 create → `TmslAlreadyExistsError`
  - [x] `test_open_nonexistent_raises` — open 不存在 → `TmslNotFoundError`
  - [x] `test_drop_dataset` — drop 后目录消失, 可重新 create
  - [x] `test_drop_recreate_with_different_params` — drop 后用不同参数 re-create
- [x] `tests/test_write_query.py` — 写入和查询测试
  - [x] `test_single_write_query` — 写 1 条, query 返回 1 条
  - [x] `test_multiple_write_query_range` — 写 100 条, query(3, 7) 返回 5 条
  - [x] `test_query_empty_range` — query 无数据的范围 → 空迭代
  - [x] `test_query_all_convenience` — `query_all()` = `list(query())`
  - [x] `test_iterator_protocol` — `for ts, data in ds.query(...)` 正确枚举
  - [x] `test_iterator_partial_consumption` — 迭代一半后丢弃, 不崩溃
  - [x] `test_write_timestamp_zero_rejected` — `write(0, ...)` → `TmslInvalidDataError`
  - [x] `test_write_negative_timestamp_rejected` — `write(-1, ...)` → `TmslInvalidDataError`
  - [x] `test_write_out_of_order_rejected` — 非连续模式下逆序写 → `TmslInvalidDataError`
  - [x] `test_flush_manual` — 写入后 `flush()`, 数据落盘
- [x] `tests/test_continuous.py` — 连续模式测试
  - [x] `test_continuous_out_of_order_write` — 允许逆序写入 (back-fill)
  - [x] `test_continuous_gap_filling` — 跳写产生 filler, 查询自动过滤
  - [x] `test_continuous_backfill_replaces_filler` — back-fill 替换 filler
  - [x] `test_continuous_duplicate_timestamp_rejected` — 重复时间戳 → 错误
- [x] `tests/test_exceptions.py` — 异常类型测试
  - [x] `test_all_exceptions_importable` — 9 个异常类均可导入
  - [x] `test_exception_hierarchy` — 所有异常继承 `TmslError`
  - [x] `test_error_message_passthrough` — 捕获异常, 验证消息包含 Rust 端描述
  - [x] `test_catch_specific_exception` — `except TmslNotFoundError` 精确捕获
- [x] `tests/test_persistence.py` — 持久化测试
  - [x] `test_reopen_after_close` — 关闭后重新 open, 数据完整
  - [x] `test_data_survives_process_restart` — 写→close (进程退出)→新进程 open→读
  - [x] `test_meta_file_invariant` — meta 文件在 reopen 时参数一致
- [x] `tests/test_multi_dataset.py` — 多数据集隔离测试
  - [x] `test_two_datasets_isolated` — 写 A 不影响 B
  - [x] `test_same_name_different_type` — 同名不同类型完全隔离
- [x] `tests/test_config.py` — 配置测试
  - [x] `test_store_config_defaults` — 所有字段默认值正确
  - [x] `test_store_config_custom` — 自定义所有字段
  - [x] `test_create_dataset_with_kwargs` — create_dataset kwargs 覆盖 store 默认
  - [x] `test_create_dataset_uses_store_defaults` — 不传 kwargs 时用 store 配置
  - [x] `test_index_continuous_kwarg` — `index_continuous=True` 启用连续模式
...
### Phase PY-8: CI/CD + 多平台 Wheel 构建 ✅ 完成

- [x] `.github/workflows/python-ci.yml` — PR/合并时 CI
  - [x] 触发条件: `push` / `pull_request` 到 `wrapper/python/` 路径变更
  - [x] 矩阵: OS = `[ubuntu-latest, macos-latest, windows-latest]`
  - [x] 步骤: checkout → setup Python (3.9-3.13 矩阵) → setup Rust → `maturin build` → `pytest`
  - [x] lint: `cargo clippy` 对 wrapper crate
  - [x] 验证 wheel 产物 `.whl` 大小合理
- [x] `.github/workflows/python-release.yml` — 发布 CI
  - [x] 触发条件: `push` tag `python-v*`
  - [x] 构建多平台 wheels (包含 manylinux / macOS universal2 / Windows)
  - [x] 上传到 PyPI (`pypa/gh-action-pypi-publish`)
- [x] 本地构建验证:
  - [x] `maturin build --release` 生成当前平台 wheel
  - [x] 验证 wheel 可 `pip install` 且 `import timslite` 成功
- [x] 文档: 在 `wrapper/python/` 或项目 README 中添加 Python 安装说明
