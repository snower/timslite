...
impl LogData {
    pub fn load() -> Self {
        ...
        let raw_lines = if log_path.exists() {
            let content = fs::read_to_string(log_path)
                .expect("Failed to read .github/data/access.log");
            content
                .lines()
    ...
    fn generate_random_lines(count: usize) -> Vec<String> {
        ...
        let paths = ["/api/users", "/api/data", "/index.html", "/api/search", "/api/login"];
        ...
        (0..count)
            .map(|_| {
                ...
                format!(r#"{} - - [26/Jun/2026:09:13:48 +0800] "GET /{} HTTP/2.0" {} {} "-" "{}""#, ip, path, status, size, ua)
            })
            .collect()
    }
...
impl BenchmarkMetrics {
    ...
    pub fn print_results(&self, label: &str) {
        ...
        println!("  Bandwidth: {:.2} KB/sec", self.write_bytes_per_sec() / 1024.0);

        println!("\nRead Performance (Sequential):");
        println!("  Operations: {} reads", self.read_sequential_ops);
        println!("  Duration: {:.3}s", self.read_sequential_duration.as_secs_f64());
        println!("  Throughput: {:.2} ops/sec", self.read_sequential_ops_per_sec());
        println!("  Bandwidth: {:.2} KB/sec", self.read_sequential_bytes_per_sec() / 1024.0);

        println!("\nRead Performance (Random):");
        println!("  Operations: {} reads", self.read_random_ops);
        println!("  Duration: {:.3}s", self.read_random_duration.as_secs_f64());
        println!("  Throughput: {:.2} ops/sec", self.read_random_ops_per_sec());
        println!("  Bandwidth: {:.2} KB/sec", self.read_random_bytes_per_sec() / 1024.0);

        println!("\nStorage Efficiency:");
        println!("  Uncompressed Size: {:.2} KB", self.total_uncompressed_size as f64 / 1024.0);
        println!("  Compressed Size: {:.2} KB", self.total_data_size as f64 / 1024.0);
