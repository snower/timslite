...
[![CI](https://github.com/snower/timslite/actions/workflows/ci.yml/badge.svg)](https://github.com/snower/timslite/actions/workflows/ci.yml)
...
- .NET wrapper 位于 [wrapper/dotnet](wrapper/dotnet)。
...
```bash
cargo add timslite@0.1.2
```

仓库开发或验证未发布改动时，可以继续使用本地 path 或 git 依赖:
...
```toml
[dependencies]
timslite = { git = "https://github.com/snower/timslite" }
```
...
发布包优先使用预编译 wheel；如果当前平台没有匹配 wheel，pip 会回退到 sdist 源码构建，需要本机 Rust toolchain 和原生编译工具链。
...
Node.js wrapper 要求 Node.js 22+，timestamp 和 identifier 使用 `bigint` 暴露以避免精度丢失。发布包包含支持平台的预编译 Node-API native binding；缺失预编译产物时会尝试源码构建。
...
```kotlin
implementation("io.github.snower:timslite:0.1.2")
```

Java wrapper 支持 Java 8+，JAR 内包含已发布平台的 native libraries。
...
.NET wrapper 目标框架为 `net8.0`，NuGet 包内包含已发布 RID 的 native assets。
...
- 可选后台维护线程。
- 可选内置 `.journal/logs` 专用 journal 存储。
- 新建 dataset 时使用的默认配置。
...
.NET wrapper 位于 [wrapper/dotnet](wrapper/dotnet)，基于 UniFFI 生成 .NET 绑定，提供 Store、Dataset、Queue、Journal 等 API。
...
.NET wrapper 详情见 [wrapper/dotnet/README.md](wrapper/dotnet/README.md)。

## 使用注意事项

- Dataset name、dataset type、queue group name 必须匹配 `^[0-9A-Za-z_-]+$`，最长 255 字节。
- `.journal/logs` 是 Store 内部保留路径，不作为普通 `DataSet` 打开。Journal 开启时通过 `journal_read` / `journal_query` / `open_journal_queue` 等专用 API 访问。
